...
export default function StagnentWater() {
  const [complaints, setComplaints] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedComplaints = await fetchComplaints('stagnentWater');
  ...
  return (
    <>
      <div className="flex m-6">
        <div style={{ width: '70%' }}>
          <MapContainer category='stagnentWater' />
        </div>
        <div className='flex flex-col items-center mr-3'>
          <h2 className='font-bold text-2xl my-3'>Stagnent Water Complaints</h2>
