...
const SRTable = ({ tableData, onSubmit }) => {
  const [editableData, setEditableData] = useState(tableData || []); // Initialize as an empty array if undefined

  useEffect(() => {
    if (tableData) {
      setEditableData(tableData); // Update the state only when tableData is available
    }
  }, [tableData]);
            ...
            <tr key={index}>
              <td className="p-2 border border-gray-300 whitespace-nowrap text-sm">
                <input
                  type="text"
                  value={row.staffName}
                  onChange={(e) => handleInputChange(e, index, "staffName")}
                  className="w-full"
                />
              </td>
              <td className="p-2 border border-gray-300 whitespace-nowrap text-sm">
